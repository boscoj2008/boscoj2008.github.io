...
Hello? In this post we will look at how to implement the popular LeNet architecture using the Sequential module of PyTorch. We will be training on the Fashion MNIST, which was created to be a drop-in replacement for the MNIST. More detainls can be found the Fashion MNIST paper [here](https://arxiv.org/abs/1708.07747).
...
LeCun in which it was applied to document character recognition. At a high-level perspective, the LeNet-
5 comprises of 3 parts namely; (i) two convolutional layers and (ii) a dense block consisting of 3
convolutional layers and sometimes uses sub-sampling or pooling layers between these layers.
...
The LeNet model was trained on a regular MNIST dataset of 60,000 images of which 50,000
were used for training and 10,000 for testing using the mean squared error as the loss function
for 20 iterations. The network achieved an error rate of 0.9%. In this task, we will be applying LeNet to the FashionMNIST data. But first, let's import some dependencies!
...
- Remember, we can train for more epochs to improve the accuracy of these CNN's, however we have to becare not to overfit.